...
const chart = d3.select("#usa-chart");
const legend = d3.select("#usa-legend");
const cityFilter = d3.select("#usa-city-filter");
...
d3.csv("./USA_air_quality_datapreproccessing_final.csv", d3.autoType).then((rows) => {
  const cleaned = rows
    .map((d) => ({
  ...
  const gradientPalette = d3.range(Math.max(allCities.length, 1)).map((_, i) =>
    d3.interpolateRgb("#001f54", "#dc143c")(allCities.length <= 1 ? 0.5 : i / (allCities.length - 1))
  );
